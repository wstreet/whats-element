...
prototype.getUniqueId = function (element, isParent) {
  ...
  if (
    !result.wid &&
    className &&
    document.querySelector(tag + className) === element
  ) {
  ...
  //location by parent
    ...
    var parentQueryResult = whatsElementPure.prototype.getUniqueId(
      parentNode,
      true
    );
  ...
  try {
    ...
    switch (type) {
      ...
      case "document.getElementById()": // 仅body有
        result = findRoot.getElementById
          ? findRoot.getElementById(queryString)
          : null;
        break;
      case "document.getElementsByName()": // 仅body有
        result = findRoot.getElementsByName
          ? findRoot.getElementsByName(queryString)[0]
          : null;
        break;
      case "document.querySelector()":
        // getElementsByTagName
        result = findRoot.querySelector
          ? findRoot.querySelector(queryString)
          : null;
        break;
      case "split":
        ...
        if (selectors.length > 1) {
          ...
          let currentTarget =
            matchedChildNodes[0] || this.getTarget(currentSelector);
          // 只查找符合要求的子元素
          if (root) {
      ...
      default:
        result =
          this.getTarget(queryString, "document.getElementById()") ||
...
prototype.compute = function (element) {
  ...
  var inView =
    viewPosition.left > 0 &&
    viewPosition.left < window.innerWidth &&
    viewPosition.top > 0 &&
    viewPosition.top < window.innerHeight;
